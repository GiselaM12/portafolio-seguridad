...
import { FaFolderPlus, FaLock, FaHourglassHalf, FaTerminal, FaDatabase, FaExternalLinkAlt, FaShieldAlt, FaBug, FaNetworkWired, FaServer, FaSearch, FaUserSecret, FaBalanceScale, FaProjectDiagram } from 'react-icons/fa';
...
const Projects = () => {
    ...
    const projectsByParcial = {
        ...
        parcial3: activities.filter(act => String(act.id) === "12" || String(act.id) === "16" || String(act.id) === "17" || String(act.id) === "18" || String(act.id) === "PR03")
            .sort((a, b) => (String(a.id).startsWith("PR") ? 1 : String(b.id).startsWith("PR") ? -1 : parseInt(a.id) - parseInt(b.id)))
            .map(act => ({
            id: act.id,
            icon: getIconForActivity(act.id),
